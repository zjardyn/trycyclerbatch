...
do
    ...
    fasta=$(find "$sample_path"/trycycler/cluster* -mindepth 1 -maxdepth 1 -name "8*fasta")
    catfasta="$sample_path"/trycycler/"$sample"_consensus_medaka.fasta
    
    echo -n "Sample: $sample, Clusters: " 
    for i in $clusters; do
      echo -n $(basename $i)" " 
    done
    echo -n ", Outputting fasta to $sample/trycycler/"$sample"_consensus_medaka.fasta"
    echo ""
